...
function Calendar() {
  ...
  const colors = ["#f1427d", "#2ed877", "#03c3db", "#fe9a0d"];
  ...
  function calculateDays() {
    ...
    })[0].getDay(); //I totally forgot about the getDay() method of the standard library. That's why I did this.
    //Now that I think about it, I could have done without the date-fns library. I literally only used it for leap year and to check the starting day of a month
    //What's done is done
    ...
    if (frequency === "Yearly") {
      ...
      if (eventsClone[selectedMonth].specific[modalOpenDay]) {
        if (eventsClone[selectedMonth].specific[modalOpenDay][selectedYear]) {
          eventsClone[selectedMonth].specific[modalOpenDay][selectedYear].push(
            newEventText
          );
        } else {
          eventsClone[selectedMonth].specific[modalOpenDay][selectedYear] = [
            newEventText,
          ];
        }
      } else {
  ...
  function deleteEvent(freq: "Yearly" | "Specific", index: number) {
    ...
    if (freq === "Yearly") {
      let tempYearlyEvents = eventsClone[selectedMonth].yearly[
        modalOpenDay
      ].filter((_, indexNumber) => indexNumber !== index);
      eventsClone[selectedMonth].yearly[modalOpenDay] = tempYearlyEvents;
      if(tempYearlyEvents.length === 0){
        delete eventsClone[selectedMonth].yearly[modalOpenDay];
      }
    } else {
      let tempSpecificEvents = eventsClone[selectedMonth].specific[
        modalOpenDay
      ][selectedYear].filter((_, indexNumber) => indexNumber !== index);
      eventsClone[selectedMonth].specific[modalOpenDay][selectedYear] =
        tempSpecificEvents;
      if(tempSpecificEvents.length === 0){
        delete eventsClone[selectedMonth].specific[modalOpenDay][selectedYear];
      }
    }
  ...
  function todaysDate(day: number) {
    const dateToday = new Date();

    if (
      dateToday.getDate() === day &&
      dateToday.getMonth() === selectedMonth &&
      dateToday.getFullYear() === selectedYear
    ) {
      return true;
    }
  ...
  return (
    <div className="calendar-comp-cont">
      <div
        className="events-modal-cont"
        style={{ display: `${modalOpenDay ? "block" : "none"}` }}
      ...
      >
        <div className="events-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <h3>
            Events for {months[selectedMonth]} {modalOpenDay}
          </h3>
          {(() => {
            const eventsData = collateEvents();
            if (
              eventsData.yearly.length === 0 &&
              eventsData.specific.length === 0
            ) {
                  ...
                  return (
                    <li key={index}>
                      ...
                      <div style={{ marginLeft: "auto" }}>
                        ...
                        <div onClick={() => deleteEvent("Yearly", index)}>
                          <svg
                  ...
                  return (
                    <li key={index}>
                      ...
                      <div
                        ...
                        onClick={() => deleteEvent("Specific", index)}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 448 512"
          ...
          <div className="new-event">
            ...
            >
              {newEventOpen ? "Submit New Event" : "Add New Event"}
            </button>
          </div>
        </div>
      </div>
            ...
            onClick={(e) => {
              ...
              <span
                ...
                style={{backgroundColor: `${
                    events[selectedMonth].yearly[day] ||
                    events[selectedMonth].specific[day]?.[selectedYear]
                      ? `${colors[day % colors.length]}`
                      : ""
                  }`}}
              >
                {day}
